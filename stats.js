export default async function handler(req, res) {
  ...
  if (!API_KEY || !GROUP_ID) return res.status(500).json({ error: 'Missing ML_API_KEY or ML_GROUP_ID' });

  const isLegacy = API_KEY.startswith ? API_KEY.startswith('mlapi-') : API_KEY[0:6] === 'mlapi-';
  const url = isLegacy
  ...
  try {
    ...
    const count = isLegacy ? (data?.active ?? data?.total ?? 0) : (data?.subscribers_count?.active ?? 0);
